make sure to make these changes and verify validate and test it with yourself

in streak 
    in mobile view make sure in the bottom where shows the month in there the current month (today's month) comes center as when clicks the strek itself as the user can slide it as per they want it ot be (should not to start with the first month) it should work as always when a user click streak in mobile view..
...
url (should work in localhost for testing)
    this should be mandatory like in the url section should be domainname.com/username === its very mandatory 
    and while shares the profile should be able to see even in an incognito mode or in any browser if that users validation is correct..

check console based error too.. and npm run error, debug, etc., 


make sure it work as its needed to be for public data transfer
